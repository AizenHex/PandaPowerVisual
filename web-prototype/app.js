...
const state = {
  ...
  // Auto-increment IDs
  nextId: { bus: 1, load: 1, gen: 1, shunt: 1, trafo: 1, line: 1 },
};
...
function newProject() {
  ...
  state.selectedId = null;
  state.selectedType = null;
  state.nextId = { bus: 1, load: 1, gen: 1, shunt: 1, trafo: 1, line: 1 };
  state.hasResults = false;
...
function loadTemplate() {
  ...
  state.selectedId = null;
  state.selectedType = null;
  state.nextId = { bus: 6, load: 5, gen: 1, shunt: 1, trafo: 2, line: 6, source: 2 };
  state.hasResults = false;
  ...
  Object.keys(state.nodes).forEach(id => {
    ...
    if (node.kind === "source") {
      ...
      const vaVal = state.hasResults && state.results.nodes[id] 
        ? `va ${state.results.nodes[id].va_degree.toFixed(1)}°` 
        : `va --`;
      lines = [
...
function loadProject() {
  ...
  state.nextId = payload.nextId || { bus: 1, load: 1, gen: 1, shunt: 1, trafo: 1, line: 1 };
  state.selectedId = null;
  state.selectedType = null;
  state.hasResults = false;
...
function setupEventListeners() {
  ...
  btnToggleLeft.addEventListener("click", () => {
    ...
    btnToggleLeft.textContent = hidden ? "Show Panel" : "Hide Panel";
  ...
  btnToggleRight.addEventListener("click", () => {
    ...
    btnToggleRight.textContent = hidden ? "Show Props" : "Hide Props";
  ...
  document.getElementById("btnZoomIn").addEventListener("click", () => {
    state.zoom = Math.min(4.0, state.zoom * 1.25);
    updateViewportTransform();
  });
